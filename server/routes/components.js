...
router.route("/order").post(async (req, res) => {
  try {
    ...
    /**
     * I couldn't figure out how to get bulkWrite working,
     * so I did them individually for now
     */
    for (let i = 0; i < components.length; i++) {
      const currLink = await LinkElement.findById(components[i]._id);
      await currLink.updateOne({ index: components[i].index });
    }
